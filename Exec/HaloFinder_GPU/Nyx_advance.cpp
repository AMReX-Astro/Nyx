                                   ...
                                   int  ncycle)
{
    ...
    if (Nyx::theActiveParticles().size() > 0)
    {
        ...
        Vector<LightConeParticle> shell_particles;
		shell_particles.clear();
		shell_particles.shrink_to_fit();

		std::string filename=Concatenate("lightcone_", int(100*(1/a_old-1)), 7);
		std::string filename_vtk="./Output/LightCones/VTK/"+filename+".vtk";
		std::string filename_bin="./Output/LightCones/SimpleBinary/"+filename+".bin";
        ...
        for (int lev = level; lev <= finest_level_to_advance; lev++)
        {
            ...
            for (int i = 0; i < Nyx::theActiveParticles().size(); i++) {
                ...
                Nyx::theActiveParticles()[i]->moveKickDrift(grav_vec_old, lev, time, dt, 
                                                            a_old, a_half, where_width, 
