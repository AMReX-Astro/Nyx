...
c ::: --------------------------------------------------------------
      subroutine cartgridminmax3d (data, dlo1, dlo2, dlo3, dhi1, dhi2, d
     &hi3,vfracdata, vfeps, dmin, dmax)
      implicit none
...
c ::: --------------------------------------------------------------
     ...
     &n,fslope,fdat,flen,voff)
      ...
      DOUBLE PRECISION fine(floi1 :fhii1 ,floi2 :fhii2 ,floi3 :fhii3, nv
     &ar)
...
c ::: ..,.......::::: strip out a fine grid slope vector
                      do 330 koff = 1, lratio 
                          icc = clo + ist*(ic-islo) + jst*(jc-jslo) + ks
     &t
                          istart = koff
...
c ::: --------------------------------------------------------------
     ...
     &lo, fbhi,lrat,nvar, crse,cloi1,cloi2,cloi3,chii1,chii2,chii3,cbl
     &o, cbhi,temp,tloi,thii)
      ...
      DOUBLE PRECISION fine(floi1 :fhii1 ,floi2 :fhii2 ,floi3 :fhii3, nv
     &ar)
      DOUBLE PRECISION crse(cloi1 :chii1 ,cloi2 :chii2 ,cloi3 :chii3, nv
     &ar)
...
c ::: --------------------------------------------------------------
     ...
     &loi1,fbloi2,fbloi3, fbhii1,fbhii2,fbhii3,nvar,lratio, crse,clo,c
     &hi,cblo,cbhi, fslo,fshi,cvloi1,cvloi2,cvloi3,cvhii1,cvhii2,cvhii
     &3, cslope,clen,fslope,fdat,flen,voff, cvc1,fvc1,cvc2,fvc2,cvc3,f
     &vc3)
...
c                                 --- break ---
     ...
     &(fn,1)+voff(fn)*fslope(fn,2)+ zoff*f
     &slope(fn,3)
580                           continue
570                       continue
571                       continue
...
c                                 --- break ---
                              ...
                              do 650 k = fbloi3, fbhii3 
                                  fn = k-fslo(3)+1
                                  fine(i,j,k,n) = fdat(fn) + xoff*fslope
     &(fn,1)+yoff*fslope(fn,2)+ voff(fn)*f
     &slope(fn,3)
650                           continue
640                       continue
641                       continue
