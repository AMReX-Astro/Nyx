                                            ...
                                            int                    where_width)
{
    ...
    auto phi=geom_test.ProbHiArray();
    GpuArray<Real,AMREX_SPACEDIM> center({AMREX_D_DECL(phi[0]-plo[0],phi[1]-plo[1],phi[2]-plo[2])});
       ...
       if (do_move == 1) 
         {
           ...
           for (int comp=0; comp < nc; ++comp) {               
               ...
               p2.pos(comp) = p.pos(comp) + dt_a_cur_inv * p.rdata(comp+1);
               p2.id()=p.id();
           }
         }
