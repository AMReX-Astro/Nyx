...
contains
  ...
  AMREX_CUDA_FORT_DEVICE subroutine nyx_eos_T_given_Re_device(JH, JHe, T, Ne, R, e, comoving_a)  &
     ...
     ! In/out variables
     integer, value, intent(in) :: JH, JHe ! stubs here
     real(rt),           intent(inout) :: T, Ne
     real(rt), value,          intent(in   ) :: R, e
     ...
     call eos_device(eos_input_re, den_eos, temp_eos, &
              xn_eos, &
              p_eos, e_eos, cv_eos, &
              dpdt_eos, dpdr_eos, dedt_eos, &
              s_eos, &
...
!     mu = (1.0d0+4.0d0*(xn_eos(2))) / (1.0d0+(xn_eos(2))+Ne)
     
!     mu=(1.0d0+4.0d0*(1-XHYDROGEN)) / (2-XHYDROGEN+0)-1

!    T  = e * (mu * mp_over_kB * gamma_minus_1)
...
!       = 5 means dens, eint    , and xmass are inputs, return temp    , etc
    ...
    real(rt), intent(inout) :: dens, temp
    real(rt), intent(inout)  :: xmass(nspec)
    real(rt), intent(inout) :: pres, eint
    real(rt) :: c_v
  ...
  ! The main interface -- this is used directly by MAESTRO
  !---------------------------------------------------------------------------
  AMREX_CUDA_FORT_DEVICE subroutine eos_device(input, dens, temp, &
                 xmass, &
                 pres, eint, c_v, &
                 dPdT, dPdR, dEdT, &
                 entropy, &
...
!       = 5 means dens, eint    , and xmass are inputs, return temp    , etc
    ...
    if (eos_assume_neutral) then
       ! assume completely neutral atoms
       do n = 1, nspecd
          ymass(n) = xmass(n)/aiond(n)
          sum_y = sum_y + ymass(n)
       enddo
    else
       ! assume completely ionized species
       do n = 1, nspecd
          ymass(n) = xmass(n)*(1.d0 + ziond(n))/aiond(n)
          sum_y = sum_y + ymass(n)
       enddo
    endif
