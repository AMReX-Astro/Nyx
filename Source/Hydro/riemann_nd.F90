...
contains
  ...
  subroutine riemanncg(ql, ql_lo, ql_hi, &
    ...
    special_bnd_lo = .false.
    !(physbc_lo(idir) == Symmetry &
    !     .or.         physbc_lo(idir) == SlipWall &
    !     .or.         physbc_lo(idir) == NoSlipWall)
    special_bnd_hi = .false.
    !(physbc_hi(idir) == Symmetry &
...
#endif
!                call nyx_eos_given_RT(rel,pl,rl, &
!                     small_temp,diag_eos(i,j,k,NE_COMP),1)!a_old)
