module trace_plm_module
  ...
  public trace_plm, trace_plm_orig
...
contains
  ...
  AMREX_CUDA_FORT_DEVICE subroutine trace_plm(lo, hi, &
    ...
    real(rt), intent(in) ::  dq(dq_lo(1):dq_hi(1),dq_lo(2):dq_hi(2),dq_lo(3):dq_hi(3),QVAR)
    ...
    fix_mass_flux_lo = .false.
    fix_mass_flux_hi = .false.
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
            ...
            azut1rght = HALF*(-ONE - spzero )*alpha0ut
            azutt1rght = HALF*(-ONE - spzero )*alpha0utt
            ...
            azut1left = HALF*(ONE - spzero )*alpha0ut
