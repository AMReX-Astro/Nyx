...
AMREX_CUDA_FORT_DEVICE  subroutine transx_on_zstates(lo, hi, &
 ...
 !!!               call reset_edge_state_thermo(qzpo, qzpo_lo, qzpo_hi, i, j, k)
                ...
                if (qzpo(i,j,k,QPRES) .lt. small_pres) then
                   ...
                   pav = HALF*(pgp+pgm)
                   du = ugp-ugm
                   pnewrz = qzp(i,j,k,QPRES) - cdtdx*(dup + pav*du*gamma_minus_1)
...
! !!!               call reset_edge_state_thermo(qzpo, qzpo_lo, qzpo_hi, i, j, k)
 ...
 !!!               call reset_edge_state_thermo(qzmo, qzmo_lo, qzmo_hi, i, j, k)
                ...
                if (qzmo(i,j,k,QPRES) .lt. small_pres) then
                   ...
                   pav = HALF*(pgp+pgm)
                   du = ugp-ugm
                   pnewlz = qzm(i,j,k,QPRES) - cdtdx*(dup + pav*du*gamma_minus_1)
...
AMREX_CUDA_FORT_DEVICE  subroutine transz_on_xstates(lo, hi, &
 ...
 !!!               call reset_edge_state_thermo(qxpo, qxpo_lo, qxpo_hi, i, j, k)
                ...
                qxpo(i,j,k,QPRES) = qxpo(i,j,k,QREINT)*gamma_minus_1
                if (qxpo(i,j,k,QPRES) .lt. small_pres) then
                   pnewrx = qxp(i  ,j,k,QPRES) - cdtdz*(dup + pav*du*gamma_minus_1)
                   qxpo(i,j,k,QPRES ) = pnewrx
                   qxpo(i,j,k,QREINT) = qxpo(i,j,k,QPRES) / gamma_minus_1
                end if
             endif
...
!             call reset_edge_state_thermo(qxpo, qxpo_lo, qxpo_hi, i, j, k)
 ...
 !!!               call reset_edge_state_thermo(qxmo, qxmo_lo, qxmo_hi, i, j, k)
                ...
                qxmo(i,j,k,QPRES) = qxmo(i,j,k,QREINT)*gamma_minus_1
                if (qxmo(i,j,k,QPRES) .lt. small_pres) then
                   pnewlx = qxm(i,j,k,QPRES) - cdtdz*(dup + pav*du*gamma_minus_1)
                   qxmo(i,j,k,QPRES ) = pnewlx
                   qxmo(i,j,k,QREINT) = qxmo(i,j,k,QPRES)  / gamma_minus_1
                end if
...
AMREX_CUDA_FORT_DEVICE  subroutine transz_on_ystates(lo, hi, &
 ...
 !!!               call reset_edge_state_thermo(qypo, qypo_lo, qypo_hi, i, j, k)
                ...
                if (qypo(i,j,k,QPRES) .lt. small_pres) then
                     pnewry = qyp(i,j  ,k,QPRES) - cdtdz*(dup + pav*du*gamma_minus_1)
                     qypo(i,j,k,QPRES ) = pnewry
                     qypo(i,j,k,QREINT) = qypo(i,j,k,QPRES) / gamma_minus_1
                  end if
               endif
...
!             call reset_edge_state_thermo(qypo, qypo_lo, qypo_hi, i, j, k)
 ...
 !!!               call reset_edge_state_thermo(qymo, qymo_lo, qymo_hi, i, j, k)
                ...
                qymo(i,j,k,QPRES) = qymo(i,j,k,QREINT)*gamma_minus_1
                if (qymo(i,j,k,QPRES) .lt. small_pres) then
                   pnewly = qym(i,j,k,QPRES) - cdtdz*(dup + pav*du*gamma_minus_1)
                   qymo(i,j,k,QPRES ) = pnewly
                   qymo(i,j,k,QREINT) = qymo(i,j,k,QPRES) / gamma_minus_1
                endif
             endif
...
AMREX_CUDA_FORT_DEVICE  subroutine transyz(lo, hi, &
 ...
 !!!               call reset_edge_state_thermo(qpo, qpo_lo, qpo_hi, i, j, k)
                ...
                if (qpo(i,j,k,QPRES) .lt. small_pres) then
                   ...
                   qpo(i,j,k,QREINT) = qpo(i,j,k,QPRES) / gamma_minus_1
               end if
               !             call reset_edge_state_thermo(qpo, qpo_lo, qpo_hi, i, j, k)
            endif
 ...
 !!!               call reset_edge_state_thermo(qmo, qmo_lo, qmo_hi, i, j, k)
                ...
                if(qmo(i,j,k,QPRES).lt.small_pres)then
                   ...
                   qmo(i,j,k,QREINT) = qmo(i,j,k,QPRES) / gamma_minus_1
                endif
             endif
!             call reset_edge_state_thermo(qmo, qmo_lo, qmo_hi, i, j, k)
...
!             call reset_edge_state_thermo(qpo, qpo_lo, qpo_hi, i, j, k)
 ...
 !!!               call reset_edge_state_thermo(qmo, qmo_lo, qmo_hi, i, j, k)
               ...
               if (qmo(i,j,k,QPRES) .lt. small_pres) then
                   pnewl = qm(i,j,k,QPRES) - pxnew - pznew
                   qmo(i,j,k,QPRES ) = pnewl + hdt* srcQ(i,j-1,k,QPRES ) / a_old
                   qmo(i,j,k,QREINT) = qmo(i,j,k,QPRES) / gamma_minus_1
                end if
...
AMREX_CUDA_FORT_DEVICE  subroutine transxy(lo, hi, &
    ...
    real(rt)         duxm,duxpm,duym,duypm
    real(rt)         pxavm,pxnewm,pyavm,pynewm
    real(rt)         compr, compl, compnr, compnl
 ...
 !!!               call reset_edge_state_thermo(qmo, qmo_lo, qmo_hi, i, j, k)
                ...
                if (qmo(i,j,k,QPRES) .lt. small_pres) then
                   ...
                   duxm = ugxpm-ugxmm
                   pxnewm = cdtdx*(duxpm + pxavm*duxm*gamma_minus_1)

                   duypm = pgypm*ugypm - pgymm*ugymm
                   pyavm = HALF*(pgypm+pgymm)
                   duym = ugypm-ugymm
                   pynewm = cdtdy*(duypm + pyavm*duym*gamma_minus_1)
                   
                   pnewl = qm(i,j,k,QPRES) - pxnewm - pynewm
                   qmo(i,j,k,QPRES ) = pnewl + hdt* srcQ(i,j,k-1,QPRES ) / a_old
                   qmo(i,j,k,QREINT) = qmo(i,j,k,QPRES) / gamma_minus_1
                end if
             endif
  ...
  use meth_params_module, only : QVAR, NVAR, NQAUX, QRHO, &
    ...
    if (ppm_predict_gammae == 0 ) then
       ...
       if (reset) then
          ...
          qedge(ii,jj,kk,QPRES) = qedge(ii,jj,kk,QREINT)*gamma_minus_1
...
end module ca_transverse_module
