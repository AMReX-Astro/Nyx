      ...
      char* argv[])
{
    ...
    Vector<int> comps_print;
    int nc = pp.countval("comps");
    comps_print.resize(nc);
    pp.getarr("comps",comps_print,0,nc);
	...
	for(int n=0;n<comps.size();++n) {
	  amrData.FlushGrids(comps[n]);
	}

	for (int n = 0; n < comps.size(); ++n) {	
	  std::cout<<"max in mmultifab for comp: "<<comps[n]<<" is: "<<mf.max(comps[n])<<std::endl;
	}
	...
	long int idx=0;
	    ...
	    std::cout<<"Probhi"<<boxSizeH<<"comps"<<comps[0]<<comps[1]<<comps[2]<<std::endl;
	      }
	...
	std::cout<<"lo,hi,len,box"<<lo<<hi<<len<<box<<std::endl;
		  ...
		  for (int y = lo.y; y <= hi.y; ++y) {
		      ...
		      if(fab_array(x,y,z,0) != bogus_flag)
			{
		      ...
		      ofs<<idx<<"\t"<<x*dx[0]<<"\t"<<y*dx[1]<<"\t"<<z*dx[2]<<"\t"<<dx[0]*dx[1]*dx[2];
			      for (int n = 0; n < comps_print.size(); ++n) {
				ofs<<"\t"<<fab_array(x,y,z,comps_print[n]); 
			      }
			      ofs<<std::endl;
			    }
			  else
		...
		for (int x = lo.x; x <= hi.x; ++x) {
		  //idx assumes ncomp=1, is local to this fab
		  //			      int idx = x+y*len.x+z*len.x*len.y-(lo.x+lo.y*len.x+lo.z*len.x*len.y);
			      if(fab_array(x,y,z,5) != bogus_flag && fab_array(x,y,z,0) != bogus_flag) 
				{
				    ...
				    for (int n = 0; n < comps_print.size(); ++n) {
				      ofs_mpi.write((char *) &(fab_array(x,y,z,comps_print[n])), sizeof(amrex::Real)); 
				    }
