...
int main(int argc, char **argv) {
    ...
    int i_density(amrData.StateNumber("particle_mass_density"));
    comps[0] = i_density;
    ...
    double mean_density = mf1.norm1() / num_cells;

    if (ParallelDescriptor::IOProcessor()) {
        printf("Mean matter density: %.14e Msun/Mpc**3\n", mean_density);
        fflush(stdout);
    }

    // Convert to mean units.
    double mean_density_inv = 1.0/mean_density;
    mf1.mult(mean_density_inv);
    ...
    ParallelDescriptor::Barrier();
    CHK(printHWM("After read/write", comm));
